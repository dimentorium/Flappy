...
def OnSysEx(fl_event):   
    ...
    if(header == b'\x7d\x11\x00'):                          #check if message from main API
        ...
        try:
            ...
            device.midiOutSysex(b'\xf0\x7d\x11\x10' + (str(result)).encode('utf-8') + b'\xf7')     #send return value via sysex back
        except Exception as err:                                #if an error occurs, send back the error from flstudio
            print("Error: ", str(err))
            device.midiOutSysex(b'\xf0\x7d\x11\x20' + (str(err)).encode('utf-8') + b'\xf7')
