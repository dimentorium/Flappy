    ...
    for name, module in sys.modules.items():                #check all imports
        if(name in APIMODULES):                             # if import is an FL Studio stub
            for name, func in inspect.getmembers(module, inspect.isfunction):
                setattr(module, name, ___remoteFL_decorator(func))  #add decorator to function
...
def ___send_command(cmd_string: str) -> bytes:
    ...
    msg = mido.Message('sysex', data=sysex_data)                        #send complete commmand string via sysec
    OUTPORT.send(msg)
    try:
...
def ___remoteFL_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        mainmodule = (func.__module__).split(".")[0]                        #extract first part of module
        valuestring = ___argumentValuesToString(args)                       #get all inputs as string
        functionstring = f"{mainmodule}.{func.__name__}({valuestring})"     #build command string
        ...
        if return_type is not type(None):                                   #type cast only if there is a return value
